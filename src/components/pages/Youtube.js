...
function Youtube() {
    ...
    const start = () => {
     ...
     const search = (query) => {
        ...
        fetch(`https://youtube.googleapis.com/youtube/v3/search?part=snippet&q=${query}&key=${API_KEY}&maxResults=30&type=video`, requestOptions)
    ...
    useEffect(() => {
        var requestOptions = {
          ...
          fetch(`https://youtube.googleapis.com/youtube/v3/search?part=snippet&q=bts&key=${API_KEY}&maxResults=30&type=video`, requestOptions)
            .then(response => response.json())
